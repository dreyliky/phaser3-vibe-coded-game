...
export class Button extends Phaser.GameObjects.Container {
    ...
    constructor(
        ...
        const bgColor = options.backgroundColor !== undefined ? options.backgroundColor : 0x333333;
        const bgAlpha = options.backgroundAlpha !== undefined ? options.backgroundAlpha : 1;
        
        // Measure text to determine default width/height if not provided
        const tempText = scene.make.text({ text, style: { fontSize, fontFamily } });
        const textWidth = tempText.width;
        const textHeight = tempText.height;
        tempText.destroy();

        const paddingX = options.padding?.x ?? 20;
        const paddingY = options.padding?.y ?? 10;
        
        const width = options.width || (textWidth + paddingX * 2);
        const height = options.height || (textHeight + paddingY * 2);
